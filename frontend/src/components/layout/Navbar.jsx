...
export default function Navbar({ onOpenMobileNav }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-slate-200/80 bg-white/85 px-4 backdrop-blur-xl sm:px-6 shadow-[0_4px_20px_-4px_rgba(15,23,42,0.03)]">
      <button
        type="button"
        onClick={onOpenMobileNav}
        className="-ml-1 rounded-lg p-2 text-slate-600 hover:bg-slate-100 lg:hidden transition-colors"
      ...
      <div className="min-w-0 flex items-center gap-3">
        <div>
          <h1 className="truncate text-[15px] font-bold text-ink tracking-tight flex items-center gap-2">
      ...
      <div className="hidden xl:flex items-center gap-2 ml-6 rounded-full bg-slate-900/[0.04] border border-slate-200/60 px-3.5 py-1 text-[11.5px] font-medium text-slate-700">
        <span className="relative flex h-2 w-2">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
          <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
        </span>
        <span className="font-semibold text-slate-800">MoSPI Live Feed</span>
        <span className="text-slate-400">|</span>
        <span className="text-slate-500 font-mono text-[11px]">XGBoost v2.5 Engine</span>
      </div>
